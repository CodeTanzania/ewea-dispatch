...
VehicleDispatchSchema.methods.preValidate = function preValidate(done) {
  ...
  const statusesOptns = common.mergeObjects(
    lodash.pick(this, 'createdAt', 'canceledAt', 'dispatchedAt', 'resolvedAt'),
    lodash.pick(this, 'pickup.dispatchedAt', 'pickup.arrivedAt'),
    lodash.pick(this, 'dropoff.dispatchedAt', 'dropoff.arrivedAt')
  );
  const statuses = eweaCommon.dispatchStatusFor(statusesOptns);
  if (statuses) {
